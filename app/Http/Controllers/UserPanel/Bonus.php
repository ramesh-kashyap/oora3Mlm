...
class Bonus extends Controller
{
  ...
  public function salary_income(Request $request)
  {
    $user = Auth::user();
  ...
  public function cashback_income(Request $request)
  {
    ...
    $status = $request->status ? $request->status : null;
    $search = $request->search ? $request->search : null;
    $notes = Income::where('user_id', $user->id)->where('remarks', 'Contract Profit Sharing')->orderBy('id', 'DESC');
    if ($search <> null && $request->reset != "Reset") {
      $notes = $notes->where(function ($q) use ($search) {
        $q->Where('rname', 'LIKE', '%' . $search . '%')
          ->orWhere('ttime', 'LIKE', '%' . $search . '%')
  ...
  public function activitiesBonus(Request $request)
  {
    ...
    $status = $request->status ? $request->status : null;
    $search = $request->search ? $request->search : null;
    $notes = Income::where('user_id', $user->id)->where('remarks', 'Same Rank Profit Sharing')->orderBy('id', 'DESC');
    if ($search <> null && $request->reset != "Reset") {
      $notes = $notes->where(function ($q) use ($search) {
        $q->Where('rname', 'LIKE', '%' . $search . '%')
          ->orWhere('ttime', 'LIKE', '%' . $search . '%')
  ...
  public function reward_income(Request $request)
{
  ...
  public function roi_income(Request $request)
  {
    ...
    $note = Income::where('user_id', $user->id)->where('remarks', 'Farming Income')->orderBy('id', 'DESC');
