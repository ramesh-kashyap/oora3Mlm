...
class DepositController extends Controller
{
    ...
    public function deposit_list(Request $request)
    {
        ...
        $status = $request->status ? $request->status : null;
        $search = $request->search ? $request->search : null;
$notes = Investment::where('status', 'Active')
            ->whereIn('is_free', [0])
            ->orderBy('id', 'DESC');
        if($search <> null && $request->reset!="Reset"){
            $notes = $notes->where(function($q) use($search){
    ...
    $notes = $notes->paginate($limit)
     ...
     public function freeactivation(Request $request)
    {
        $limit = $request->limit ? $request->limit : paginationLimit();
        $status = $request->status ? $request->status : null;
        $search = $request->search ? $request->search : null;
    ...
    $notes = $notes->paginate($limit)
        ...
        $this->data['deposit_list'] =  $notes;
        $this->data['search'] = $search;
        $this->data['page'] = 'admin.deposit.zeropin';
        return $this->admin_dashboard();
    }
...
{
    $deposit = Investment::where('user_id', $user_id)
        ->where('status', 'Active')
        ->where('is_power', 1)
        ->first();

    if (!$deposit) {
        return back()->with('error', 'No active record found to delete.');
    }
    ...
    $notes = $notes->paginate($limit)
        ...
        $this->data['deposit_list'] =  $notes;
        $this->data['search'] = $search;
        $this->data['page'] = 'admin.deposit.powerreport';
        return $this->admin_dashboard();
    }
